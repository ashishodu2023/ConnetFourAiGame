...
def MiniMaxAlgo(board, depth, maximizing_player):
    """
    :param board: Grid
    ...
    if maximizing_player:
        ...
        for col in range(COLS):
            if IsValidMove(board, col):
                ...
                eval = MiniMaxAlgo(board, depth - 1, False)
        ...
        for col in range(COLS):
            if IsValidMove(board, col):
                ...
                eval = MiniMaxAlgo(board, depth - 1, True)
...
def IsBestMove(board):
    ...
    for col in range(COLS):
        if IsValidMove(board, col):
            row = CheckEmptyRow(board, col)
            MakeMove(board, col, 'X')
            eval = MiniMaxAlgo(board, 4, False)  # Adjust the depth here
            if eval > max_eval:
                max_eval = eval
                best_col = col
            board[row][col] = ' '  # Undo the move
